...
def api_translate_file():
    ...
    input_filename = f"upload_{uuid.uuid4().hex[:8]}{file_ext}"
    ...
    try:
        ...
        # 修改2：根据文件类型调用不同的处理函数
        if file_ext == '.txt':
            # 需要实现 translate_txt 函数
            from modules.file_handler import translate_txt
            output_path = translate_txt(input_path, translate_fn)
            download_name = "译文.txt"
        else:  # .docx
